<template>
    <div class='card'>
        ...
        <div class='card-body'>
            ...
            <form @submit.prevent="updatePemilik">
                ...
                <div class='form-group'>
                    ...
                    <select name="" class="form-control" v-model="pemilik.barang_id">
                        <option v-for="barang in barangs" v-bind:value="barang.id" :selected="barang.id ==
                        pemilik.barang_id">
                            {{ barang.nama }}
                        </option>
                    </select>
...
export default {
    ...
    created() {
        ...
        let uriPemilik = `/api/pemilik/${this.$route.params.id}`
        this.axios.get(uriPemilik).then(response => {
    ...
    methods: {
        updatePemilik(e) {
            if (
                ...
                this.$swal.fire({
                    title: 'Success',
                    text: "Pemilik updated successfully",
                    icon: 'success',
                    timer: 1000
                })
                let uri = `/api/pemilik/${this.$route.params.id}`;
                this.axios.put(uri, this.pemilik).then((response) => {
